...
from app.utils.pdf_draw import collect_ruling_lines, insert_section_text, restore_ruling_lines
from app.utils.pdf_fonts import SpanStyle, resolve_font, setup_page_fonts

__all__ = [
    "ANY_HEADER_RE", "normalize_header", "profile_section",
    "collect_ruling_lines", "restore_ruling_lines", "insert_section_text",
]
    ...
    for ln in lines:
        for s in ln.get("spans", []):
            ...
            if txt in bullet_chars:
                bx = s["bbox"][0]
                after = [sp for sp in ln.get("spans", []) if sp.get("text", "").strip() and sp["bbox"][0] > bx + 1]
                tx = min((sp["bbox"][0] for sp in after), default=margin_x + 12.0)
                return txt, bx, tx
    return "•", margin_x, margin_x + 12.0
...
def profile_section(
    ...
    first_body_top = all_lines[header_idx + 1]["bbox"][1] if header_idx + 1 < len(all_lines) else None
    ...
    header_style = SpanStyle(
        font_raw=h_font_raw, font_name=h_font_name, font_obj=h_font_obj,
        size=header_span.get("size", 12.0), color=header_span.get("color", 0),
        x=header_span["bbox"][0], flags=header_span.get("flags", 0),
    )
    ...
    body_lines = [ln for ln in all_lines[header_idx + 1:] if ln["bbox"][1] < y_bottom]
    if not body_lines:
        return None

    all_body_spans = [s for ln in body_lines for s in ln["spans"] if s.get("text", "").strip()]
    ...
    dominant_font, dominant_size = max(font_size_counts, key=font_size_counts.get) if font_size_counts else ("helv", 10.0)
    body_spans = [s for s in all_body_spans if s.get("font", "") == dominant_font and abs(s.get("size", 10) - dominant_size) < 0.5]
    ...
    body_style = SpanStyle(
        font_raw=dominant_font, font_name=body_font_name, font_obj=body_font_obj,
        size=dominant_size, color=body_color, x=page_left_margin,
    ...
    if not subheader_spans:
        for ln in body_lines:
            if ("|" in ln["text"] or ":" in ln["text"]) and len(ln["text"].split()) <= 15:
                subheader_spans.extend(s for s in ln["spans"] if s.get("text", "").strip())
    ...
    if subheader_spans:
        ...
        subheader_style = SpanStyle(
            font_raw=sh_font_raw, font_name=sh_font_name, font_obj=sh_font_obj,
            size=sh_ref.get("size", dominant_size), color=sh_ref.get("color", body_color),
            x=page_left_margin, flags=sh_ref.get("flags", 0),
        )
    else:
        sh_font_name, sh_font_obj = resolve_font(font_registry, dominant_font, is_bold=True)
        subheader_style = SpanStyle(
            font_raw=dominant_font, font_name=sh_font_name, font_obj=sh_font_obj,
            size=dominant_size, color=body_color, x=page_left_margin,
            flags=body_spans[0].get("flags", 0) | (1 << 4) if body_spans else (1 << 4),
        )

    # Bullet character and indentation positions
    bullet_char, bullet_x, text_x = _find_bullet(body_lines, page_left_margin)
    if bullet_x == page_left_margin and text_x == page_left_margin + 12.0:
        doc_lines = [ln for b in text_dict.get("blocks", []) if b.get("type") == 0 for ln in b.get("lines", [])]
        bullet_char, bullet_x, text_x = _find_bullet(doc_lines, page_left_margin)

    # Line spacing
    body_ys = sorted(set(round(s["origin"][1], 1) for s in body_spans if "origin" in s))
    if len(body_ys) >= 2:
