...
LLM-based resume writer. Syncs GitHub repos, runs LLM-ranked matching, tailors resume, returns a new one. Agentic skill chain (parse JD → match projects → write resume → ATS check) with SSE streaming.
...
```bash
# 1. Clone and enter
git clone https://github.com/<owner>/<repo> && cd samuel
...
User → Next.js → FastAPI → Orchestrator
                               ├── JD Parser (LLM)
                               ├── Project Matcher (LLM-ranked)
                               ├── Resume Writer (LLM)
                               └── ATS Checker (deterministic)
                     → SSE stream → User sees live progress
```

PDF output uses PyMuPDF in-place rewrite as primary. WeasyPrint render is fallback only when no source PDF bytes exist.

The orchestrator runs three LLM skills plus a deterministic ATS check. Each skill is defined by a prompt template (.md file) and a runner (.py class). Results flow through the chain: JD text → structured requirements → ranked projects → rewritten resume → ATS report.
